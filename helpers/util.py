...
async def generate_card_embed(card, user, signed = False, rarity = "basic"):
    ...
    logger.info(f"User {user["id"]} got a copy of {card_id} {rarity}{" signed" if signed else " normal"} {card["fd_bundle"]} {card["fd_member"]} - {copies} total")
...
async def upgrade_card(card, stars):
    ...
    if total_value < 2 ** stars:
        ...
        if await run_query(query, False) is not None:
            ...
            logger.info(f"User {card["fd_cowner"]} upgraded {card["fd_display"]} {details["fd_type"]} {details["fd_bundle"]} {details["fd_member"]} to {rating} stars")
            return embed, file, caption
        else:
            logger.error(f"There was an issue after user {card["fd_cowner"]} tried to upgrade {from_rating} {card["fd_display"]} {details["fd_type"]} {details["fd_bundle"]} {details["fd_member"]} to {rating} stars")
